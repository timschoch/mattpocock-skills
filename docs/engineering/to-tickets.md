...
| A real tracker (GitHub, Linear) | Native blocking links, plus a native sub-issue link to the parent spec or epic | Any ticket whose blockers are done is on the **frontier** and can be grabbed |
...
Known upstream, and fixed in this fork: `to-tickets` now links every ticket under its parent as a native sub-issue by default, rather than only "where convenient". Upstream it has been reported across a dozen runs and several models, [most fully in issue #554](https://github.com/mattpocock/skills/issues/554), and is worse on Codex than on Claude. `gh` has supported this natively since v2.94: `gh issue create --parent <n>`, and `gh issue edit <parent> --add-sub-issue <n>` after the fact.
