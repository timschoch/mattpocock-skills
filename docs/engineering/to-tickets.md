...
- **A real tracker (GitHub, Linear)** → one issue per ticket, the edges as native blocking links (or sub-issues). Every ticket is linked under its parent epic or spec as a native sub-issue, so the parent shows the whole breakdown in its sub-issue list, not only in prose. Any ticket whose blockers are all done is on the **frontier** and can be grabbed — so several agents can run at once.
