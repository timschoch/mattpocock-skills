...
- **[orchestrate](./orchestrate/SKILL.md)** — **(fork-only, not in upstream)** Run an already-ticketed feature across parallel `/implement` agents, one per sub-issue on its own worktree; the orchestrator sequences, relays, merges and cleans up.
